...
import type { WebRoute, WebServer } from './web-server.ts'

/** cordis.yml 插件行使用的插件名。 */
export const name = 'dsh-qianxun-tab'
...
/** 一条提示词（与浏览器半区 QianxunPrompt 结构一致，跨端以 JSON 交换）。 */
interface QianxunPrompt {
  id: string
  name: string
  content: string
  created_at: number
...
/** 落盘结构。`rev` 每次写入自增，供浏览器半区判断是否需要刷新。 */
interface PromptStore {
  version: number
  rev: number
  updatedAt: number
  prompts: QianxunPrompt[]
}
...
function normalizePrompt(raw: unknown): QianxunPrompt | undefined {
  ...
  const now = Date.now()
  const createdAt = typeof source['created_at'] === 'number' && Number.isFinite(source['created_at'])
    ? source['created_at']
    : now
...
function normalizeList(raw: unknown): QianxunPrompt[] {
  if (!Array.isArray(raw)) return []
  const byId = new Map<string, QianxunPrompt>()
  for (const item of raw) {
    const prompt = normalizePrompt(item)
    if (prompt !== undefined) byId.set(prompt.id, prompt)
  }
  return [...byId.values()].slice(-MAX_PROMPTS)
}
...
function emptyStore(): PromptStore {
  return { version: STORE_VERSION, rev: 0, updatedAt: 0, prompts: [] }
}
...
async function readStore(): Promise<PromptStore> {
  ...
  try {
    ...
    const updatedAt = typeof parsed['updatedAt'] === 'number' && Number.isFinite(parsed['updatedAt'])
      ? parsed['updatedAt']
      : 0
    return { version: STORE_VERSION, rev, updatedAt, prompts: normalizeList(parsed['prompts']) }
  } catch {
    return emptyStore()
  }
}
...
async function writeStore(prompts: QianxunPrompt[]): Promise<PromptStore> {
...
/** 排队一次全量替换写入。 */
function enqueueWrite(prompts: QianxunPrompt[]): Promise<PromptStore> {
...
async function handlePrompts(req: IncomingMessage, res: ServerResponse): Promise<void> {
  ...
  if (req.method === 'PUT' || req.method === 'POST') {
    ...
    const prompts = normalizeList(body['prompts'])
    const store = await enqueueWrite(prompts)
    writeJson(res, 200, {
      ok: true,
      rev: store.rev,
      updatedAt: store.updatedAt,
...
export function apply(ctx: Context): void {
  ...
  if (webServer === undefined || webServer === null) {
    ctx.logger.warn(
      'dsh-qianxun-tab: webServer 服务不可用，提示词端点未注册'
      + '（浏览器半区将回落 localStorage：换浏览器/换端口会看不到提示词）',
    )
    return
  }
  const route: WebRoute = { kind: 'exact', path: ROUTE_PATH, handler: handlePrompts }
  ctx.effect(() => {
    try {
