...
async function fetchServer() {
    try {
        ...
        if (body.ok !== true || !Array.isArray(body.prompts))
            return undefined;
...
export function ensurePromptsLoaded() {
    loaded ??= load();
    return loaded;
}
/**
...
export function updatePrompt(id, patch) {
    ...
    if (patch.name !== undefined)
        cur.name = patch.name.trim() || cur.name;
    if (patch.content !== undefined)
        cur.content = patch.content.slice(0, MAX_CONTENT);
...
/** 复制提示词到剪贴板。返回是否成功。 */
export async function copyPrompt(id) {
