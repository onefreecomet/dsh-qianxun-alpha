...
export declare const QIANXUN_BASE: string;
...
/** 取批次列表（引擎返回最新在前）。 */
export declare function listBatches(): Promise<QianxunBatch[]>;
/** 取单批详情（含逐条指标）。 */
export declare function getBatch(batchNo: string): Promise<QianxunBatchDetail>;
/** 取单批完整结果。 */
export declare function getBatchResult(batchNo: string): Promise<QianxunBatchDetail>;
/** 取单 alpha 的 BRAIN check 详情（PROD_CORRELATION + 全部 checks）。
 * qianxund 端有 60s TTL 缓存。失败时抛 Error，UI 需要捕获并降级显示。 */
export declare function getAlphaCheck(alphaId: string): Promise<QianxunAlphaCheckResponse>;
...
export declare function getAlphaPnl(alphaId: string): Promise<QianxunPnlResponse>;
 ...
 * 二次调用秒出，无 BRAIN 网络往返。 */
export declare function getCachedPnl(alphaId: string): Promise<QianxunCachedPnlResponse>;
/** 单个缓存条目元信息。 */
...
export declare function getPnlCacheStatus(): Promise<QianxunPnlCacheStatus>;
/** 一键清空本地 PnL 缓存（需要 X-Clear-Token: yes-i-know 确认）。 */
export declare function clearPnlCache(): Promise<{
    cleared: number;
}>;
...
export declare function getActiveAlphas(): Promise<QianxunActiveAlphasResponse>;
/** PnL 缓存批量回填任务：启动请求体 */
...
export declare function startBackfillPnls(req?: QianxunBackfillStartRequest): Promise<QianxunBackfillStartResponse>;
/** 轮询回填任务进度 */
export declare function getBackfillStatus(jobId: string): Promise<QianxunBackfillState>;
...
export declare function getAlphaProdCorr(alphaId: string): Promise<QianxunProdCorrResponse>;
/** 对批次执行控制动词（pause / resume / cancel）。 */
export declare function actOnBatch(batchNo: string, action: QianxunAction): Promise<unknown>;
/** 断点续跑：POST /api/jobs/<batch>/resume。 */
export declare function resumeBatch(batchNo: string): Promise<unknown>;
/** 提交新批次。`expressions` 为表达式字符串或配置对象数组。 */
export declare function submitBatch(body: {
    settings: unknown;
    expressions: unknown[];
    name?: string;
    producer?: string;
...
export declare function getConfig(): Promise<QianxunConfig>;
/** POST /api/config —— 调并发 / 批大小。body 只传要改的项。 */
export declare function setConfig(body: {
    concurrent?: number;
    batch_size?: number;
}): Promise<QianxunConfig>;
/** GET /api/quota —— 今日提交数 / 每日配额。 */
export declare function getQuota(): Promise<QianxunQuota>;
...
/** 读池子（纯本地，不打 BRAIN）。 */
export declare function getAlphaPool(): Promise<QianxunPoolResponse>;
/** 改池子：action = add | remove | clear | set。 */
export declare function mutateAlphaPool(action: 'add' | 'remove' | 'clear' | 'set', ids?: string | string[]): Promise<QianxunPoolResponse>;
/** 写某条 alpha 的备注（纯本地）。注意引擎的 action 是 note，不是改池子成员。 */
export declare function setAlphaPoolNote(alphaId: string, note: string): Promise<QianxunPoolResponse>;
/** 同步：拉取整池（或指定 ids）的指标 / 相关性 / 状态。 */
export declare function syncAlphaPool(ids?: string[]): Promise<QianxunPoolSyncResponse>;
/** 账号当日新增 ACTIVE 数（每天本地 12:00 重置，吃引擎侧 5min 缓存）。 */
export declare function getActiveStats(force?: boolean): Promise<QianxunActiveStats>;
/**
 ...
 * agent / 脚本不得代为调用。
 */
export declare const CONFIRM_SUBMIT_TOKEN = "yes-i-know";
export declare function submitAlphas(ids: string[], opts?: {
    pollOnly?: boolean;
}): Promise<QianxunSubmitStart>;
/** 查提交任务进度。 */
export declare function getSubmitJob(jobId: string): Promise<QianxunSubmitJob>;
//# sourceMappingURL=qianxun.d.ts.map